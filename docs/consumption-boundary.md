# Engineering Assurance v0.3.0 consumption boundary

`v0.3.0` is the first immutable Rust-port beta. It is deliberately a
...
Use the immutable `v0.3.0` tag, never a branch name or a pull-request head.
For a Rust consumer, pin the repository dependency by tag:

```toml
engineering-assurance = { git = "<Engineering Assurance repository remote>", tag = "v0.3.0" }
```

For the configuration module or CLI, check out `v0.3.0` from this repository's
...
with the title prefix `v0.3.0 boundary:`. Include the tag, resolved commit,
