...
| engineering-assurance | 0.3.2 candidate | planned git tag `v0.3.2` | Rust-native shared semantics, compatibility mapping, the corpus gate, classifier, and ix-flow lifecycle host |

The three external tools are released artifacts. EA v0.3.2 becomes one when
...
| engineering-assurance | check out the previous tag `v0.3.1` | the current-toolchain matrix and new schema contracts; retained external evidence and ix-flow state are unchanged |
...
They are informational until the v0.3.2 tag is cut; the observer no longer
