...
| FR-017 | FR-017-AC-7 | TC-121 | ✅ Rust manifest classifier backed by locally authored, non-authoritative fixture schema and registry; the retained manifest's conformance to the FR-035 schema and upstream edge registry is not asserted (PLAT-992) |
...
| FR-026 | FR-026-AC-4 | TC-169 | ✅ Adversarial steering values produce the identical decision-rule verdict as none at all |
...
| TC-169 | Two independently-parsed MeasurementPlan objective blocks sharing one decision rule -- one with no steering fields, the other with adversarial weight: 0, a near-zero value_half_life and budget: 0 -- parse to equal decision rules, agree on direction/comparator consistency, and produce the identical holds() verdict for every probed estimate | Unit | P0 | FR-026-AC-4 | ✅ adversarial-vs-plain verdict-parity cases passing |
| TC-170 | The MeasurementPlan skeleton carries a valid objective with weight, value_half_life and budget, a "Steering Fields" section stating they are advisory only, and validates against the schema with no onboarding-checklist warning | Unit | P0 | FR-026-AC-5 | ✅ `tests/test_module.py` skeleton case passing |
