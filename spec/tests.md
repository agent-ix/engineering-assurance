...
| FR-014 | FR-014-AC-6 | TC-188 | ✅ Grammar-selected requirement-test audit: doc-comment accept/absent/empty/malformed cases and attribute-only import findings passing |
...
| TC-188 | The requirement-test source audit is told one trace grammar. Under `DocComment`, `/// Trace:` lines with well-formed identifiers pass and an absent line, empty list, or malformed entry each yields its own finding; under either grammar the other grammar's form is not accepted, and import-shape findings appear only under `Attribute` | Static | P0 | FR-014-AC-6 | ✅ `tests/source_audit.rs` doc-comment and grammar-exclusivity cases passing |
