...
| TC-122 | A valid synthetic executable and retained input run once through the public Rust executor and produce one typed completed result carrying exact sha256-jcs request identity, executable/caller/adapter provenance, monotonic timing, bounded raw evidence, validated output artifact, and caller-adapter observation | Integration | P0 | FR-019-AC-1 | 🚧 pending implementation |
| TC-123 | Every closed producer-request field changes its JCS identity; list order is significant while declared sets normalize; structurally invalid input mints no identity; identity-bearing digest/capability/adapter/cancellation mismatches refuse before launch; the valid child observes no shell or ambient environment | Property | P0 | FR-019-AC-2, FR-019-CON-1 | 🚧 pending implementation |
| TC-124 | Unavailable, refused, failed, timed-out, malformed-response, containment-failure, cancelled, and completed results remain distinct; launched states retain bounded raw evidence where observable and only completed carries the caller-owned typed observation | Property | P0 | FR-019-AC-3 | 🚧 pending implementation |
| TC-125 | Exact wall-clock, stream, input-byte, output-count/byte, and concurrency boundaries are admitted and the next value is invalid or terminates; admitted non-zero exits reach the adapter; ordinary descendants are reaped; an escaping-descendant mutant becomes containment-failure under the explicitly cooperative profile | Integration | P0 | FR-019-AC-4, FR-019-CON-4 | 🚧 pending implementation |
