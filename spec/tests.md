...
| TC-122 | A valid synthetic executable and input run once through the public Rust executor and produce one typed completed result carrying exact request identity, executable digest, producer provenance, and caller-adapter observation | Integration | P0 | FR-019-AC-1 | 🚧 pending implementation |
| TC-123 | Every producer request field changes its identity; invalid protocol, procedure, paths, digests, inputs, environment names, response binding, and exact/over resource limits refuse before launch; the valid child observes no shell or ambient environment | Property | P0 | FR-019-AC-2, FR-019-CON-1 | 🚧 pending implementation |
| TC-124 | Unavailable, refused, failed, timed-out, malformed-response, containment-failure, cancelled, and completed results remain distinct; only completed carries the caller-owned typed observation | Property | P0 | FR-019-AC-3 | 🚧 pending implementation |
| TC-125 | Exact wall-clock, stdout, stderr, input-byte, and concurrency boundaries are admitted and the next value refuses or terminates; all descendants, permits, and temporary state are released on completion, timeout, cancellation, malformed response, and failure | Integration | P0 | FR-019-AC-4 | 🚧 pending implementation |
