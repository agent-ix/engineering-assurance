...
| TC-119 | Registry-derived snapshot binds the provider candidate separately from every active external AP/MP canonical repository, clean candidate commit, path, blob digest, provider/schema/skeleton digests, validation outcome, and owner disposition; the registry refuses its own provider repository, duplicated submodule checkouts are deduplicated, and no unapproved valid-to-invalid transition occurs | Integration | P0 | FR-015-AC-5 | 🚧 pending implementation |
...
| TC-122 | Consumer registry accepts multiple uniquely pathed artifact entries beneath each unique canonical repository entry and rejects empty artifact lists, unknown versions/classes, non-canonical ordering, duplicate repository identities, duplicate repository-relative paths, population-digest mismatches, exact registry-byte changes, unattributed exclusions, and pending, half-attributed, agent-attributed, or wrong-population owner review; accepted review names the human and date | Property | P0 | FR-015-AC-7 | 🚧 pending implementation |
...
| TC-119, TC-122 | Registry | one/many/empty artifacts per repository; active/inactive/skeleton/template/quarantined; canonical/reordered/duplicate repo and repository-relative path; matching/mismatched/changing population and exact-input digest; pending/accepted/half-attributed/agent-attributed/wrong-population owner review | one human-accepted canonical population or explicit refusal; exclusions require owner and reason |
