...
| FR-014 | FR-014-AC-7 | TC-190 | ✅ `tests/feature_matrix.rs` manifest/list equality and `make rust-features` compile of each feature alone passing |
...
| TC-190 | The library compiles with default features disabled and with each Cargo.toml capability feature (all but `default` and `full`) enabled alone; the Makefile's derived feature list equals the manifest's declared set | Compile | P0 | FR-014-AC-7 | ✅ `tests/feature_matrix.rs` passing; run locally via `make rust-features`, not wired into CI |
