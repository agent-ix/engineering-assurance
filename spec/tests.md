...
| FR-017 | FR-017-AC-9 | TC-136 | ✅ Provider responses omit absent optional scenario metadata; the consuming host accepts the described catalogue |
...
| TC-136 | A `describe` response omits `title` and `canary` from the emitted bytes for every scenario that carries neither, still emits `canary` for the canary scenario and `use_case` for every scenario, and emits no null anywhere in the described catalogue | Test | P0 | FR-017-AC-9 | ✅ emitted-byte omission backed |
