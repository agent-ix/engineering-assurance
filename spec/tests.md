...
| FR-017 | FR-017-AC-3 | TC-111 | 🚧 Membership, content-rights tree, manifest, and npm stage/cleanup/publication-refusal slices backed; bounded Rust archive and installed-bundle adapter pending |
| FR-017 | FR-017-AC-4 | TC-112 | 🚧 npm lifecycle and content-rights gates are declarative Rust dispatch; remaining host-configuration census pending |
...
| FR-018 | FR-018-AC-2 | TC-113 | 🚧 Content-rights same-revision parity and direct tree cutover backed; package-audit same-revision cutover and final deletion pending |
| FR-018 | FR-018-AC-3 | TC-114 | 🚧 Content-rights dispatch rollback and reapplication backed; aggregate rollback population pending |
...
| TC-111 | Each package, rights, manifest, integration, and publication-refusal capability has independent positive and negative subcases matching the retained gate; npm stage/cleanup covers missing, linked, special, non-portable, oversized, excessive, pre-existing, changed, exact-copy, rollback, idempotent-absence, direct-JSON, and hook-empty-stdout states; the content-rights tree adapter covers exact Git selection, tracked/untracked/ignored entries, gitlinks, links, special files, protected tokens, repository-root identity, deterministic findings, and every exact/over resource boundary; the package-audit adapter covers direct bounded builder/installer processes, preflight-absent and exact post-process npm staging cleanup, exact archive selection, validated regular file/directory kinds, safe unique member names, exact/over archive bytes, total entries, member bytes, and aggregate expanded bytes, independent report/archive allowlist agreement, every content-rights category without disclosure, exact license/private metadata, offline installs, thin host discovery, canonical/pilot YAML equivalence, installed links/special files/escapes, and cross-format canonical byte identity | Property | P0 | FR-017-AC-3 | 🚧 package-audit adapter pending; preceding membership, content-rights, manifest, and npm lifecycle slices backed |
| TC-112 | Package-manager and host files contain declarative dispatch only; npm lifecycle hooks invoke the pinned local Rust CLI in explicit hook mode without embedded staging/refusal semantics or contamination of npm output; `make test` invokes the Rust content-rights tree adapter and `make package-audit` invokes the Rust package-audit adapter after their respective same-revision correspondence; local qualification, real-agent evaluation, publication, and release operations remain explicit manual actions | Static | P0 | FR-017-AC-4, FR-017-CON-2 | 🚧 npm lifecycle and content-rights dispatch backed; package-audit dispatch and remaining host-configuration census pending |
| TC-113 | Removal refuses mismatched revisions, incomplete parity, and direct invocations that still use the old path; content-rights removal additionally requires exact full-tree status/finding correspondence and an independently passing Rust tree and package-audit dispatch at one candidate revision, followed by removal of every executable import or subprocess reference to the deleted Python paths | Property | P0 | FR-018-AC-2, FR-018-CON-1 | 🚧 content-rights parity/cutover backed and deletion correctly refused while package audit imports Python; package-audit cutover and deletion pending |
| TC-114 | A failed cutover can restore the previous invocation without changing historical bytes; the content-rights and package-audit dispatches are separately reverted while their retained implementations remain present before the final deletion step | Property | P0 | FR-018-AC-3, FR-018-CON-2 | 🚧 content-rights revert/reapply backed; package-audit and aggregate rollback populations pending |
