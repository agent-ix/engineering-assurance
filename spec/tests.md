...
| TC-077 | The committed corpus reproduces from its recorded sources, and states plainly when it is skipped | Integration | P0 | FR-011-AC-9 | ✅ `tests/python_port/corpus_reproduction.rs` invokes the pinned `agent-ix/qa-corpus` builder rather than reimplementing it; stated skip where the source repositories are absent |
...
| TC-174 | `onboard.js --help` and `-h` print usage on stdout and exit 0; an unknown argument, `--repo` without a path, and a repeated `--repo` print the error and usage on stderr and exit 2; none prints a report | Unit | P1 | FR-001-AC-8 | ✅ `tests/python_port/onboarding_script.rs` usage cases passing |
...
`tests/python_port/corpus_reproduction.rs` invokes rather than reimplementing: copying
...
remain in `tests/python_port/migration_contract.rs`, which reads the contract document
