...
| FR-024 | FR-024-AC-1 | TC-154 | ✅ Protected-apparatus schema accept/refuse cases passing |
...
| FR-024 | FR-024-AC-6 | TC-159 | ✅ Skeleton, onboarding skill, and warning-free onboarding checklist passing |
...
| TC-154 | The MeasurementPlan frontmatter schema accepts no protected_apparatus and a list of safe relative paths and globs, and rejects an empty list, a repeated entry, a non-list, a non-string entry and every unsafe entry of the shared case table (empty, absolute, empty segment, `.` or `..` segment, partial `**`, forbidden character) | Unit | P0 | FR-024-AC-1 | ✅ schema accept/refuse cases passing |
| TC-155 | The schema accepts each negative-control kind at every stage, leaves negative_controls optional below gate and requires it at gate, and rejects an empty list, an unknown or missing kind, a missing or empty description, an extra key, a repeated control and a bare kind string | Unit | P0 | FR-024-AC-2 | ✅ schema accept/refuse and gate-stage cases passing |
| TC-156 | The Rust ApparatusPath accepts and refuses the shared case table with distinct typed errors through construction, parsing and deserialization, the schema pattern agrees on every case, and ProtectedApparatus refuses an empty or repeated list, compares as a set and round-trips sorted | Unit | P0 | FR-024-AC-3 | ✅ construction, parsing, deserialization, pattern agreement and set cases passing |
