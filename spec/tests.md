...
| TC-176 | A bounded output-tree request changes identity with its path or required bit and rejects nested fixed outputs or overlapping trees | Unit | P0 | FR-019-AC-4 | ✅ request-structure cases passing; Linux artifact observation pending |
...
| TC-187 | A Rust fixture copied beside a sibling fixture and pinned by absolute path reports its own resolved executable path and `argv[0]` as the pinned real path, re-executes itself through that path, and runs its sibling from the same directory, and a `#!` script pinned by absolute path sees that path as `$0`, each completing through the executor | Integration | P0 | FR-019-AC-13 | ✅ self-path, `argv[0]`, self re-execution, sibling execution and `#!` script `$0` cases passing |
