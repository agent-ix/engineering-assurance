...
| TC-199 | `ContentDigest` (the one SHA-256 evidence identity) pins the bare lowercase hex for empty, short and past-64-KiB inputs through bytes, streaming reader, file and incremental hashing, and through each migrated call site (evidence output digest, PGM-01 source digest, corpus retained-artifact digest, executor snapshot, governing file and runtime identity, retained transcript, history digest, package staging digest); `of_file` refuses links, non-regular and absent paths and the reader refuses over-ceiling and cancelled reads; the algorithm is carried by the value; the `sha256:<hex>` form round-trips and the parser refuses an unknown algorithm, uppercase, wrong length and a missing prefix while the bare parser refuses the prefixed form; the serialized form is the bare hex string | Unit | P0 | FR-014-AC-11 | ✅ `src/content_digest.rs`, `tests/evidence_parity.rs`, `tests/semantics_parity.rs`, `tests/compatibility_corpus.rs`, `tests/producer_execution.rs` and the host-module unit tests passing |
| TC-200 | A source scan of `src/` finds `sha2`/`Sha256` only in the digest module and a documented allowlist of different-purpose sites, compared for equality so a private helper and a stale entry both fail; the scan itself detects a helper and ignores comments | Static | P0 | FR-014-AC-11 | ✅ `tests/digest_identity_audit.rs` passing |
| FR-014 | FR-014-AC-11 | TC-199, TC-200 | ✅ `ContentDigest` unit, wire-form and per-call-site golden cases, and the `tests/digest_identity_audit.rs` allowlist scan passing |
