...
| TC-194 | The module manifest exports its five artifact types and both campaign types through the semantic contract: each export is a declared type with a `data_schema`, every digest equals the file's SHA-256, every exported schema is 2020-12 with an `$id` under the manifest version, each artifact type's `data_schema` is the same file as its `frontmatter_schema_ref`, the artifact schemas use no draft-07 form and no `format` and carry the one documented RFC 3339 pattern, and the digested files carry no CR and are excluded from line-ending conversion | Static | P1 | FR-003-AC-7 | ✅ `tests/semantic_exports.rs` passing; `EA_BLESS=1 cargo test --test semantic_exports -- --ignored bless` rewrites `$id` and digests after a version bump |
| TC-195 | Each artifact schema, built at default validator options, gives the verdict the original draft-07 schema gave over every skeleton, its top-level single-field mutations, the decision-rule, `margin_mode` and interval-level cases, the retired-plan cases and nested `review_by` date-time cases, with calendar validity of a date-time not asserted | Unit | P1 | FR-003-AC-8 | ✅ `tests/semantic_exports.rs` recorded-verdict case passing |
