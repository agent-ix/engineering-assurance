...
| FR-014 | FR-014-AC-10 | TC-198 | ✅ `tests/duplicate_crates.rs`: the default graph and every capability feature alone resolve no duplicate crate version except the listed `manifest` and `full` residue, compared for equality |
...
| TC-198 | `cargo tree --duplicates` (normal and build edges, every target) over the default graph and each capability feature alone (`--no-default-features`) finds no crate at more than one version, except `syn` under `manifest`; under `--features full` the duplicated set equals the listed residue exactly, so an unlisted new duplicate and a listed but resolved one both fail; a consumer with only `source-audit` resolves none | Static | P0 | FR-014-AC-10 | ✅ `tests/duplicate_crates.rs` passing; mutation-checked by emptying the `manifest` residue |
