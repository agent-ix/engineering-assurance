...
| FR-014 | FR-014-AC-8 | TC-196 | ✅ `tests/feature_matrix.rs`: `source-audit` and `manifest` dependency graphs contain no cap-std, clap, tar, zip or flate2 |
...
| TC-196 | The normal-dependency graph of `source-audit` alone (default features off) contains none of `cap-std`, `clap`, `tar`, `zip`, `flate2`, and the `manifest` graph (which names `structured-yaml`) reaches `yaml_serde` and `jsonschema` but none of those crates, so each capability feature lists only what its module compiles against | Static | P0 | FR-014-AC-8 | ✅ `tests/feature_matrix.rs` passing (`cargo tree` per feature) |
