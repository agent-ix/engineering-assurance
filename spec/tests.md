...
| FR-017 | FR-017-AC-3 | TC-111 | 🚧 Membership, content-rights tree, manifest, and npm stage/cleanup/publication-refusal slices backed; archive and installed-bundle integration pending |
...
| FR-018 | FR-018-AC-2 | TC-113 | 🚧 Content-rights same-revision parity and direct tree cutover backed; package-audit consumer blocks checker removal |
...
| TC-111 | Each package, rights, manifest, integration, and publication-refusal capability has independent positive and negative subcases matching the retained gate; npm stage/cleanup covers missing, linked, special, non-portable, oversized, excessive, pre-existing, changed, exact-copy, rollback, idempotent-absence, direct-JSON, and hook-empty-stdout states; the content-rights tree adapter covers exact Git selection, tracked/untracked/ignored entries, gitlinks, links, special files, protected tokens, repository-root identity, deterministic findings, and every exact/over resource boundary | Property | P0 | FR-017-AC-3 | 🚧 membership, content-rights tree, manifest, and npm lifecycle slices backed; archive and installed-bundle integration pending |
| TC-112 | Package-manager and host files contain declarative dispatch only; npm lifecycle hooks invoke the pinned local Rust CLI in explicit hook mode without embedded staging/refusal semantics or contamination of npm output; `make test` invokes the Rust content-rights tree adapter after same-revision correspondence; local qualification, real-agent evaluation, publication, and release operations remain explicit manual actions | Static | P0 | FR-017-AC-4, FR-017-CON-2 | 🚧 npm lifecycle and content-rights dispatch backed; remaining host-configuration census pending |
| TC-113 | Removal refuses mismatched revisions, incomplete parity, and direct invocations that still use the old path; content-rights removal additionally requires exact full-tree status/finding correspondence and an independently passing Rust dispatch at one candidate revision | Property | P0 | FR-018-AC-2, FR-018-CON-1 | 🚧 content-rights parity/cutover backed and deletion correctly refused while package audit imports Python; aggregate pending |
| TC-114 | A failed cutover can restore the previous invocation without changing historical bytes; the content-rights dispatch is reverted while the retained checker remains present before its deletion step | Property | P0 | FR-018-AC-3, FR-018-CON-2 | 🚧 content-rights revert/reapply backed; aggregate rollback population pending |
