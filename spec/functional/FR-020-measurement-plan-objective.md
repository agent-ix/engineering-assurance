...
- The schema and the Rust `Objective` type SHALL refuse an unknown direction,
  ...
  `objective` other than `direction`, `bound`, and the FR-026 steering fields
  (`weight`, `value_half_life`, `budget`).
...
- The `objective` SHALL be part of the plan's measurement definition: adding,
  removing, or changing it (its `direction` or `bound`; the FR-026 steering
  fields are excluded from the definition) while `definition_version` stays the same SHALL be
