...
| FR-014-AC-8 | Every library module has its own capability feature that names only the optional dependencies the module compiles against, plus any other capability whose module it imports; the `full` umbrella is every capability plus the binary-only crates and the `serde_json/arbitrary_precision` flip. The `source-audit` feature alone resolves none of `cap-std`, `clap`, `tar`, `zip`, `flate2`. The feature table is in the crate documentation of `src/lib.rs`. | Test (TC-196) |
