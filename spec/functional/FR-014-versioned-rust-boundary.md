...
| FR-014-AC-10 | The dependency graph a consumer resolves has no duplicate crate version, measured on normal and build edges over every target (what `cargo deny` counts with `multiple-versions = "deny"`, dev-dependencies excluded): the default graph and every capability feature alone, `source-audit` included, resolve none, except the named residue, which is `syn` 2 and 3 under `manifest` (`jsonschema`'s macro subtree) and, under `full`, `syn`, `io-lifetimes` and the `windows-*` crates (`jsonschema` and `cap-std` subtrees). The residue is documented, with its owner, in the README, and a listed crate that stops being duplicated fails the check so the list cannot go stale. `campaign` uses one digest generation: its `sha1` (git object ids, algorithm unchanged) is the release on the same RustCrypto `digest` as `sha2`. | Test (TC-198) |
