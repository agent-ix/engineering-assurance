...
| FR-014-AC-6 | The source audit's requirement-test role takes an explicit `Attribute` or `DocComment` trace grammar. Under `DocComment`, a test carrying `/// Trace:` with well-formed comma-separated identifiers yields no finding, and an absent line, an empty list, or a malformed entry each yields a typed finding; import-shape findings are never reported. Under `Attribute`, a doc-comment trace does not satisfy the gate, and a `#[trace(...)]` source is not accepted under `DocComment`. | Analysis (TC-189) |
