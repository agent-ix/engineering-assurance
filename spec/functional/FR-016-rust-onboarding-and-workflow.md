...
| --- | --- | --- |
| FR-016-AC-1 | For the accepted onboarding fixtures, Rust returns the declared complete sorted inventory, status, recommendation, and artifact path for existing, absent, conflicting, malformed, unavailable, unjustified, incomplete-boundary, and valid-authoring cases without executing another implementation; duplicate-key or merge-key artifact identities remain malformed; absolute or parent-traversing targets, symlink escapes, existing destinations, invalid staged artifacts, malformed requests, and unsupported artifact types refuse without publishing bytes. | Property (TC-105) |
