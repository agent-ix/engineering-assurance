...
- A structurally valid request produces one
  ...
  an opaque digest binds the process's observed CPU affinity. Such observations
  cannot establish comparability across boots or different CPU sets. Raw
...
| FR-019-AC-8 | For a launched Linux invocation, the executor samples a complete, bounded host context before launch and binds it into canonical result identity; stable machine identity or explicitly boot-scoped identity is represented only by a domain-separated digest, boot-scoped identity additionally binds observed CPU affinity, and unavailable host context is explicit and cannot substantiate comparability. | Test (TC-175) |
