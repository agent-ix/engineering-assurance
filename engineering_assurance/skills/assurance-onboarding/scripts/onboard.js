...
const measurementRecordChecklist = {
  collection: {
    ...
    always: [
      ...
      "configDigest — non-empty string (a NEW collection must use sha256:<64 LOWERCASE hex>, below)",
    ...
    schemaVersion2VerificationStack: [
      ...
      "  toolchains — when present: an object whose node, rust and python entries are each a non-empty string, null, or absent (a stored record may omit toolchains entirely — requiring it is a NEW-collection-intake rule, below)",
    ...
    newCollectionIntakeOnly: [
      ...
      "configDigest must be sha256:<64 LOWERCASE hex>",
      "verificationStack.toolchains must be present and set at least one of {node, rust, python}",
      "each observation's metric must resolve to an ACTIVE MeasurementPlan discovered under spec/assurance/ OR assurance/ in the target repo (both are searched — discovery.rs; a file counts only when its frontmatter says type: MeasurementPlan), at the SAME definitionVersion, and observation.planId must equal that plan's id. Plans are keyed by metric, so keep one plan per metric",
      "the plan's statistical_design is checked against the observation: a plan that states repetitions requires population.repetitions, or the record is refused with QM-POPULATION-UNSTATED",
